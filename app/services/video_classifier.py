...
class VideoClassifier(BaseModel):
    __path = fr'{os.path.dirname(os.path.abspath(__file__))}\models\video_classifier.sav'
    ...
    @staticmethod
    def get_instance():
        if(VideoClassifier.__instance is None):
            nn = VideoClassifier()
            VideoClassifier.__instance = nn

        return VideoClassifier.__instance
